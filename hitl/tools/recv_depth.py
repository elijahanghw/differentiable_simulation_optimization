#!/usr/bin/env python3
"""
recv_depth.py — decode the depth frames depth_hitl publishes.
...
class Frame:
    ...
    data:         np.ndarray    # (rows, cols) float32 tensor, or uint16 mm
...
def decode(packet: bytes) -> Frame:
    ...
    (magic, version, flags, seq, payload_type, _reserved, rows, cols, payload_bytes,
    ...
    if payload_type == PAYLOAD_POOLED_F32:
    ...
    return Frame(seq=seq, stale=bool(flags & FLAG_STALE_POSE), payload_type=payload_type,
                 ...
                 pose_age_us=pose_age, render_us=render_us,
...
def main() -> int:
    ...
    try:
        while True:
            ...
            frames.append(np.asarray(f.data))
            poses.append(np.concatenate([f.pos, f.quat]))
            times.append([f.send_time_us, f.pose_time_us, f.pose_age_us, f.render_us])

            if args.show:
                print(f"\nseq {f.seq}  {f.rows}x{f.cols}  "
                      f"{'STALE' if f.stale else 'ok'}  pose {f.pose_age_us / 1000:.1f} ms old  "
                      f"render {f.render_us} µs")
                ...
                print(f"\rseq {f.seq}  {1000.0 / max(g.mean(), 1e-6):5.2f} Hz  "
                      ...
                      f"render {f.render_us:5d} µs  pose {f.pose_age_us / 1000:5.1f} ms  "
    ...
    if args.save and frames:
        np.savez_compressed(args.save, depth=np.stack(frames), pose=np.stack(poses),
                            timing=np.array(times, np.int64))
